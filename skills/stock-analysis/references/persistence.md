...
5. GitHub Pages is the chosen live presentation when available. Do not claim a URL is live until the deployment is successful and the page loads. If Pages setup needs an account action, complete and commit the site first, then explain the exact remaining setup.
