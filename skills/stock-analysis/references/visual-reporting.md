...
Each series has a supported metric key (see `NAMES`), `unit`, `currency`, `scale`, `basis`, `period_kind: quarter|annual`, `source_id`, `source_row`, and chronological `points`: `{period: "Q2 2026"|"FY 2025", period_ending, value, state}`. Null means absent, never zero. P/E/P/S must be positive or null. Preserve the source distinction between not meaningful and unavailable. An optional bilingual `note` (`{en, ro}`) is shown under that chart; use it when the provider definition materially differs from the issuer's, e.g. debt that includes brokerage funding liabilities.

`market` stores separate dated `overview_quote`, `stats_quote`, `forecast_quote` records with `price`, `observed_at`, `session` (`regular_close`, `intraday`, `pre_market`, `after_hours`; shown as labelled), `source_id`. `range` has low/high, ISO `low_date`/`high_date` or null (shown when supplied), method and source_id. `ma200` has value, method and source_id. Never describe a provider intraday 52-week range as a daily-closing-price range. Do not fabricate dates of extrema. Derive distance to an average only when the price/average snapshot dates agree.
...
Use two roomy columns on desktop, one on phones. Keep a consistent title, cadence/unit, visible values, chart, direct chart interaction, source table and short interpretation. Date labels stay horizontal and sparse; every observation remains available through chart interaction and the table. Normalize scales before sharing axes. Fill missing period slots so line segments cannot bridge a missing quarter. Losses in a single series use a loss color; grouped series preserve their distinct colors above and below zero.

Run the maintenance suite, then inspect actual chart layouts and controls. Verify negative values, missing periods, absent components, annual fallbacks, long translated labels, and both themes. Test source disclosures, hover, pinning, touch and keyboard interaction, export, mobile overflow, and unchanged archive hashes. If the preview environment cannot access local files, do not claim local visual QA; inspect the authorized deployed report instead.

Inspect periods directly on the chart, without a dropdown: hover previews, click or tap pins, leaving restores the pinned period (or latest), and Latest / Escape resets. Use full-height period targets for small bars, lines and gaps. Keep one keyboard-focusable slider per chart with arrow keys, Home/End and meaningful period/value announcements. Highlight the period in the plot and source table and label the headline values with its date. Preserve axes and historical values; missing observations remain unavailable with no invented point.
